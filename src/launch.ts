 ...
 */
export function buildArgs(profile: Profile, serversPath: string, pluginDirs: string[] = []): string[] {
  const args: string[] = []
  ...
  // skills bundle(s) — each `--plugin-dir` takes one path (repeatable), so it's safe before
  // the greedy `--allowed-tools`.
...
export async function execClaude(
  ...
  const args = buildArgs(profile, serversPath, pluginDirs)
